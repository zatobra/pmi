...
@Component({
  selector: "upload-samsung-sales-achievement",
  templateUrl: "./upload-samsung-sales-achievement.component.html",
  styleUrls: ["./upload-samsung-sales-achievement.component.scss"],
  providers: [
...
export class UploadSamsungSalesAchievementComponent implements OnInit {
...
monthList =[
  ...
  uploadData(post) {
    ...
    if (
      ...
      (post.selectedRegionUp == -1 || !post.selectedRegionUp)
    ) {
      this.loadingData = false;
      this.toastr.error(
        "Plz select a single " +
      ...
      this.loadingData = true;
      this.httpService.uploadSamsungSalesAchievement(formData).subscribe((data) => {
        if (data) {
          debugger;
          this.response = data;
