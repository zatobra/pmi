...
@Component({
  selector: "upload-snd-kpis-component",
  templateUrl: "./upload-snd-kpis-component.html",
  styleUrls: ["./upload-snd-kpis-component.scss"],
  providers: [
...
export class UploadSndKpisComponent implements OnInit {
...
//formData.append("cityId", post.selectedRegionUp);
    ...
    formData.append("yearVal", moment(this.year.value).format("YYYY") );
    formData.append("monthVal", this.monthValue);
 ...
 if (this.form.get("avatar").value == null) {
      ...
      this.loadingData = true;
      this.httpService.uploadSndKpisData(formData).subscribe((data) => {
        if (data) {
          debugger;
          this.response = data;
  ...
  viewData(){
   ...
   // formData.append("cityId", post.selectedRegionUp);
    ...
    // formData.append('startDate', post.date);
    console.log("fulldate: ", moment(this.year.value).format("YYYY") + '-' + this.monthValue);
    const yearVal= moment(this.year.value).format("YYYY");
    const monthVal = this.monthValue;
  ...
  let obj = {
    ...
    this.httpService.getSndKpisData(obj).subscribe(
        
      (data: any) => {
        console.log("getSndKpisData data: ",data );
        if (data) {
          this.kpisData = data;
        }
        if(data?.length==0){
  ...
  downloadFile(file) {
    ...
    const type = file.key;
    let data: any = {};
    const fileTitle = 'Snd Kpis Data';

    data = this.kpisData; 
    var csvOptions = {
      headers: Object.keys(this.kpisData[0])
    };
    if (type === 'csv') {
